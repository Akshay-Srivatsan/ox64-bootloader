...
static void put_hex(uint64_t x, bool seen_first, bool is_long) {
  const char *data = "0123456789abcdef";
  for (int i = is_long ? 15 : 7; i >= 0; i--) {
...
void vprintk(const char *format, va_list arg) {
  ...
  while (*s != 0) {
    ...
    if (d == 'l') {
      ...
      switch (d) {
        case 'x':
          put_hex(va_arg(arg, uint64_t), false, true);
          break;
        default:
          uart_putc('%');
          uart_putc('l');
    ...
    switch (d) {
      case '%':
        ...
        break;
      case 'x':
        put_hex(va_arg(arg, unsigned), false, false);
        break;
      case 'd':
      ...
      case 'p':
        uart_puts("0x");
        put_hex(va_arg(arg, unsigned), true, true);
        break;
      case 'b':
        uart_puts("0b");
